...
**Secrets.** Keys, tokens or credentials that are logged, echoed into CI output, committed, sent to a client bundle, or placed in a URL. Framework build-time prefixes (`NEXT_PUBLIC_`, `VITE_`, `REACT_APP_`) make a value public by definition — check nothing sensitive acquired one.
