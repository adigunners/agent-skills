...
def write_pack(directory: Path, name: str, *, lane: str, reviewers: list[str], paths: list[str], ack=None):
    lines = [
        "---",
        f"domain: {name}",
        f"lane: {lane}",
...
class ReviewPlanTests(unittest.TestCase):
    def setUp(self):
        ...
        write_pack(
            ...
            paths=["src/api/**", "src/lib/account*"],
            ack=["personal-data handling"],
        )
        write_pack(
            self.packs,
            "automation",
    ...
    def test_overlapping_packs_union_roles_and_raise_lane(self):
        result = plan.build_plan(
            ...
            ["src/app/settings/page.tsx", "src/api/account/route.ts"],
            [],
        )
        self.assertEqual(result["lane"], "critical")
        self.assertEqual(
        ...
        self.assertEqual(result["human_ack"], ["personal-data handling"])
    ...
    def test_hotfix_with_match_plays_every_role(self):
        result = plan.build_plan(plan.load_packs(self.packs), ["src/app/page.tsx"], ["hotfix"])
        self.assertEqual(result["lane"], "critical")
...
class ReviewGateTests(unittest.TestCase):
    def plan(self, *, lane="standard", ack=None):
        return {
            "lane": lane,
            "matched_paths": [] if lane == "exempt" else ["src/api/account/route.ts"],
            "human_ack": ack or [],
        }
    ...
    def test_blocked_verdict_cannot_hide_in_separator_variants(self):
        for line in ("BLOCKED — 2 findings", "BLOCKED - 2 findings", "**BLOCKED: 2 findings**"):
    ...
    def test_human_ack_comes_from_plan(self):
        ack_plan = self.plan(ack=["published policy"])
        passed, message = self.evaluate(ack_plan, label_text="reviewed:five-a-side", body="CLEAR TO MERGE")
        self.assertFalse(passed)
        self.assertIn("published policy", message)
        passed, _ = self.evaluate(
            ack_plan,
...
class ReviewStateTests(unittest.TestCase):
    def setUp(self):
        ...
        self.plan_file.write_text(json.dumps({"lane": "standard", "reviewers": ["spec"]}))
