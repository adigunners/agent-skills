...
def evaluate(
    ...
    *,
    ...
    if base == "main" and head_ref == "staging":
        return True, "Promotion staging -> main: exempt; its commits were reviewed before promotion."
    if BOT_RE.search(actor):
        return True, f"Automated PR by {actor}: exempt; dependency and secret checks still apply."

    applied_labels = labels(label_text)
    if "review:override" in applied_labels:
        if not OVERRIDE_RE.search(body):
            return False, "The review:override label needs an OVERRIDE REASON: line in the PR body."
        return True, "Review overridden deliberately with a recorded reason."

    lane = plan["lane"]
    if lane == "exempt":
        return True, "No review-pack paths matched - no five-a-side model review is required."

    path_lines = "\n".join(str(path) for path in plan["matched_paths"])
    if event == "push":
        if merged_pr:
            return True, f"Arrived on {base} via pull request #{merged_pr}; the PR gate already judged it."
        return False, (
    ...
    if verdict == "INCOMPLETE":
        return False, "Review was INCOMPLETE. Re-run the missing reviewer before merging."
    if verdict == "BLOCKED":
        return False, "Review is BLOCKED. Resolve blocking findings or record a deliberate override."
    if plan["human_ack"] and "human-ack:confirmed" not in applied_labels:
        reasons = "; ".join(str(reason) for reason in plan["human_ack"])
        return False, f"Human acknowledgement is required for: {reasons}. Add human-ack:confirmed after reading the change."
