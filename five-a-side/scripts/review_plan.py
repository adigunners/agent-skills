#!/usr/bin/env python3
"""Build a deterministic, repository-specific five-a-side review plan."""
...
def build_plan(packs: list[Pack], paths: list[str], signals: list[str]) -> dict[str, Any]:
    ...
    matched = [
        pack
        for pack in packs
        if any(_matches(path, glob) for path in clean_paths for glob in pack.paths)
    ]
    matched_paths = sorted(
        {
            path
            for path in clean_paths
            if any(_matches(path, glob) for pack in matched for glob in pack.paths)
        }
    )
