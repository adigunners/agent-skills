...
- **Consent, and its default.** A toggle that starts on is a decision to collect by default. Check the default, not just the presence of the control — where the project has a standing decision on a default — messaging consent starting **off**, say — a diff that flips it is a finding regardless of intent.
